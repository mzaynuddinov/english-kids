...
class _AlphabetPageState extends State<AlphabetPage>
  ...
  late final ScrollController strip;
  late int index;
  ...
  void initState() {
    ...
    strip = ScrollController();
    pulse = AnimationController(
        vsync: this, duration: const Duration(milliseconds: 900))
      ..repeat(reverse: true);
    WidgetsBinding.instance.addPostFrameCallback((_) => _scrollStrip(index));
  }
  ...
  void _setIndex(int next, {bool fromPage = false}) {
    if (next < 0 || next > 25) return;
    if (index != next) {
      setState(() => index = next);
    }
    widget.onIndex?.call(next);
    if (!fromPage && controller.hasClients) {
  ...
  void _scrollStrip(int next) {
    if (!strip.hasClients) return;
    const itemExtent = 38.0;
    final viewport = strip.position.viewportDimension;
    final target = (next * itemExtent) - (viewport / 2) + 16;
    strip.animateTo(
      target.clamp(0.0, strip.position.maxScrollExtent),
      duration: const Duration(milliseconds: 220),
      curve: Curves.easeOut,
    );
  }

  void _go(int next) => _setIndex(next);
  ...
  Widget build(BuildContext context) {
    return ListenableBuilder(
      ...
      builder: (context, _) {
        ...
        final done =
            alphabetLetters.where((l) => letters.contains(l.letter)).length;
              ...
              children: [
                    ...
                    children: [
                        ...
                        children: [
                          Text('$done / 26 ҳарф',
                              style: const TextStyle(
                                  fontWeight: FontWeight.w800, color: cyan700)),
                          const Spacer(),
                          if (done >= 26)
                            const Text('🎉 Алифбо пурра омӯхта шуд!',
                                style: TextStyle(
                      ...
                      ClipRRect(
                        ...
                        child: LinearProgressIndicator(
                            value: done / 26, minHeight: 8, color: emerald500),
                      ),
                    ],
                  ),
                ),
                Expanded(
                  child: PageView.builder(
                    ...
                    itemCount: 26,
                    onPageChanged: (value) => _setIndex(value, fromPage: true),
                    ...
                    children: [
                      SizedBox(
                        ...
                        child: index > 0
                            ? FadeTransition(
                                opacity:
                                    Tween(begin: 0.45, end: 1.0).animate(pulse),
                                child: IconButton(
                                  tooltip: 'Ҳарфи қаблӣ',
                                  onPressed: () => _go(index - 1),
                                  icon: const Icon(Icons.chevron_left_rounded,
                                      size: 32, color: cyan700),
                                ),
                              )
                      ...
                      return GestureDetector(
                        ...
                        child: CircleAvatar(
                          ...
                          backgroundColor: selected
                              ? teal600
                              : (ok
                                  ? emerald600.withValues(alpha: 0.18)
